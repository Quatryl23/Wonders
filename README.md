<img width="1000" height="256" alt="wonders_header" src="https://github.com/user-attachments/assets/74d5129e-ab46-4854-8cc0-eec61063c609" />

| [Download as Datapack](https://www.curseforge.com/minecraft/data-packs/wonders) | [Download as Mod](https://www.curseforge.com/minecraft/mc-mods/wonders) |
|------------------------------------------------------------------|----------------------------------------------------------------------------------------|

# The Seven Wonders of the Ancient World in Minecraft
- Seven new, rare structures of impressive size
- Special challenges, including minigames and bossfights
- Unique items with new abilities

*The structures are built to match the true size and (mostly) the appearance of their historical counterparts. However, the interiors are often more vaguely adopted to make the exploration more interesting and add fun gameplay.*

## The Great Pyramid of Giza
<img width="960" height="540" alt="pyramid" src="https://github.com/user-attachments/assets/49dcbe66-3c65-4cad-82ae-309ae220a3c7" />

Once you've entered this giant piece of sandstone, you better find a way to navigate through the endless corridors. And don't even think about mining your way to the King's chamber: You don't want to disturb the peace of the dead!

<details>
  <summary><strong>Challenge</strong></summary>
  Find a way through a giant maze inside the pyramid to find the burial chamber
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>
    
  - Hidden traps
  - Husk and Parched guards
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>
  
  - Gold
  - Emeralds
  - Diamonds
  - Ancient Debris
  <details>
    <summary><strong>Unique item</strong></summary>
    
  *Coming soon*
  </details>
</details>

## The Hanging Gardens of Babylon
<img width="960" height="540" alt="gardens" src="https://github.com/user-attachments/assets/188dbbcd-4adb-4b71-8f63-2e9a88e31d1e" />

This terrace complex is said to contain more plants than the Nether and End dimension combined (probably way more)! And it's also the most unwieldy place in the desert, so why not play a game of Hide and Seek there?

<details>
  <summary><strong>Challenge</strong></summary>
  Meet the villager at the top and play Hide and Seek (Find them five times!)
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>
    
  - Wasted time
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>
  
  - Plants of all kinds
  - Gardening tools
  - Ancient Debris
  <details>
    <summary><strong>Hoe of the Hanging Gardens
    </strong></summary>
    
  - Highly durable diamond hoe
  - Farmland tilled with the Hoe of the Hanging Gardens will cause improved plant growth in a 9x9 block area:
    - Crops will spread to adjacent open farmland
    - Crops will overgrow adjacent crops with a slightly lower stage of growth
  - Tilling farmland will cause other affected farmland up to 100 blocks away to stop their improved plant growth
  </details>
</details>

## The Lighthouse of Alexandria
<img width="960" height="540" alt="lighthouse" src="https://github.com/user-attachments/assets/865d52b0-0ac4-40ae-ad60-9a98d9b1dfd4" />

No wonder there are so many shipwrecks in Minecraft considering how rarely a lighthouse is actually there to guide the ships. Now all the drowned sailors must be very angry at this lighthouse for not being where they've sunk.

<details>
  <summary><strong>Challenge</strong></summary>
  Reach the light on top of the lighthouse
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>
    
  - Angry Drowned
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>
  
  - Belongings of the former lighthouse keepers
  - Stored fuels
  - Old food
  - Ancient debris
  <details>
    <summary><strong>Lighthouse Keeper's Spyglass
    </strong></summary>

  - Through this spyglass, you can see:
     - A green square indicating that a village or shipwreck is nearby
    
     or
     - A red cross leading the way to the nearest village or shipwreck
  </details>
</details>

## The Colossus of Rhodes
<img width="960" height="540" alt="colossus" src="https://github.com/user-attachments/assets/892bdc88-0204-4bdd-831f-549e41e01512" />

From what I see, I'd say this one's to remember a successful raid on a trial chamber

<details>
  <summary><strong>Challenge</strong></summary>
  
  Finish the parkour leading up to the torch shining
  above the beach of Rhodes
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>
    
  - Gravity (Falling)
  - Gravity (Falling, heavy copper)
  - Vexing enforcers of gravity

  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>

  - Copper
  - Ancient debris
  <details>
    <summary><strong>Light of Rhodes
    </strong></summary>

  - Campfire
  - When placed down, illuminates a large area and grants nearby players positive effects:
    - Speed
    - Jump boost
    - Haste
    - Strength
  - After use, takes some time until it can be used again
  </details>
</details>

## The Statue of Zeus at Olympia
<img width="960" height="540" alt="statue" src="https://github.com/user-attachments/assets/963e8004-0a6c-4af5-bda9-c5e2542dde51" />

Yes, what you see is not the statue. But it's in that temple, I promise! And I wouldn't mine it even if it was, let's say, pure gold. You don't want to mess with Zeus, he might have permission level 3!

<details>
  <summary><strong>Challenge</strong></summary>
  
  Disassemble the whole golden statue
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>

  - Zeus' defenders: Copper golems that attack players around them:
    - Slow the player down
    - Damage the player
    - Set the player on fire
    - Hit the player with lightning strikes
  - Zeus power pulling you towards his defenders
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>

  - Blocks of gold
  - Ancient debris
  <details>
    <summary><strong>Zeus' Copper Defender
    </strong></summary>

  - Copper golem statue
  - When placed down, attacks nearby enemies with lightning
  - Hostile towards undead mobs, illagers, creepers and other player-hostile mobs
  </details>
</details>

## The Temple of Artemis at Ephesus
<img width="960" height="540" alt="temple" src="https://github.com/user-attachments/assets/662896c1-8ae4-42db-821c-52b88a68d1b2" />

When you approach this temple, be aware that Artemis is a great hunter and archer. But luckily, this is only a temple with an inanimate statue, right? Right!?

<details>
  <summary><strong>Challenge</strong></summary>
  
  Approach the statue of Artemis and kill her when she awakens
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>

  - Artemis' raw archery talent
  - Living corpses of Artemis' victims
  - Sneaky surprise attacks
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>

  - Arrows
  - Remains of Artemis' hunted victims
  - Ancient Debris
  <details>
    <summary><strong>Artemis' Bow
    </strong></summary>

  - Highly durable bow
  - Arrows shot with Artemis' Bow evoke a rain of arrows when a target is hit
  </details>
</details>

## The Mausoleum of Halicarnassus
<img width="960" height="540" alt="mausoleum" src="https://github.com/user-attachments/assets/a2ae9da9-796d-4a9a-a524-90e7ff3e2aae" />

Try to find an entrance until you realize it was blocked years ago. Smart move of a dead man who is too lazy to build a giant maze around his tomb. But is that enough to stop potential tomb raiders? I would defend my grave with my life!

<details>
  <summary><strong>Challenge</strong></summary>
  
  Survive multiple waves of skeletons whose rest in peace was disturbed
  <br>
  <br>
  <details>
    <summary><strong>Dangers</strong></summary>

  - Skeletons with powerful weapons
  - A barrier preventing you from escaping death
  - Mausolos and Artemisia, the masters of the Mausoleum
  </details>
</details>
<details>
  <summary><strong>Loot</strong></summary>

  - Bones
  - Damaged military equipment (Armors, Weapons)
  - Ancient Debris
  <details>
    <summary><strong>Jar of Xerxes
    </strong></summary>

  - Pot with a skull on the front
  - When placed down, villagers nearby repeatedly put emeralds inside
  - Villagers also gain experience by doing this but can't level up this way
  - Jar of Xerxes can be broken to collect the emeralds and placed down again afterwards
  </details>
</details>
